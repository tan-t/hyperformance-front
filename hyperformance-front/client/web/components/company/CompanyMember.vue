...
export default {
  ...
  beforeRouteEnter (route, redirect, next) {
    var companyUrl = route.params.companyUrl;
    Promise.all([getMembers(companyUrl),getInvitations(companyUrl)])
    .then(reses=>{
      var members = reses[0];
      var invitations = reses[1];
      next(vm=>vm.initialize(companyUrl,members,invitations));
    }).catch(err=>{
      // ??
    });
  },
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    .then(reses=>{
      var members = reses[0];
      var invitations = reses[1];
      this.initialize(companyUrl,members,invitations);
      next();
    }).catch(err=>{
      // ??
    });
  ...
  methods:
  {
    initialize:function(companyUrl,members,invitations) {
    ...
    onClickSendInvitation: function (e) {
        ...
        io.socket.post(getUrl('invite',this.companyUrl),model,(res,stat)=>{
